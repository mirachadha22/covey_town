...
import { ChatMessage, LineData, UserLocation } from '../CoveyTypes';
...
export default class CoveyTownController {
  ...
  /* A map of currelty active whiteboards, key is the room name and value is canvas base64 encoded string data */
  private _canvasHash: Map<string, string> = new Map<string, string>();
  ...
  removePlayerFromConversationArea(player: Player, conversation: ServerConversationArea): void {
    ...
    if (conversation.occupantsByID.length === 0) {
      ...
      // if the conversation area is desotryed, the current canvas data for the conversation are should be cleared
      this._canvasHash.delete(conversation.label.concat(this._coveyTownID));

      this._listeners.forEach(listener => listener.onConversationAreaDestroyed(conversation, this.coveyTownID));
    } else {
      this._listeners.forEach(listener => listener.onConversationAreaUpdated(conversation));

      const curConvSockets: Socket[] = [];
      conversation.occupantsByID.forEach(playerID => {
        const session = this.getSessionByPlayerID(playerID);

        if (session && session.socket) {
          curConvSockets.push(session.socket);
        }

      });
      this._listeners.forEach(listener => listener.onSocketRoomUpdated(conversation.label.concat(this._coveyTownID), curConvSockets));
    }
  }
  ...
  /**
   * Send all listeners LineData and the room that should receive the data
   *
  ...
  onDrawing(data: LineData, room: string): void {
    this._listeners.forEach(listener => listener.onDrawing(data, room));
  }

  /**
   * Updates the canvasHash map with the identifying key and canvas state value
   *
   * @param data The base64 encoded string represenation of the canvas 
   * @param curConvLabel The key value or room string
   */
  onUpdate(data: string, curConvLabel: string): void {
    this._canvasHash.set(curConvLabel, data);
  }

  /**
   * Sends all listeners the room string and list of sockets. Each listener will
   * make sure that their repspective socket is in the list of sockets and is connected
   * to the socket room
   *
   * @param roomName string representing a room identifier
   * @param curConvPlayerIDs list of player IDs in a conversation area
   */
  onJoin(roomName: string, curConvPlayerIDs: string[]): void {
    // calculates a list of sockets from the list of player IDs
    const curConvSockets: Socket[] = [];
    curConvPlayerIDs.forEach(playerID => {
      const session = this.getSessionByPlayerID(playerID);
    ...
    // pass on the current canvas state from the cavasHash map
    const currentConvCanvas = this._canvasHash.get(roomName);

    this._listeners.forEach(listener =>
      listener.onJoin(roomName, curConvSockets, currentConvCanvas!),
    );
  }
  ...
  /**
   * Fetch a player's session based on the provided player ID. Returns undefined if the
   * session token is not valid.
   *
   * @param playerID string of a player's ID
   */
