...
import { UserLocation, LineData } from '../CoveyTypes';
...
describe('CoveyTownController', () => {
  ...
  describe('townSubscriptionHandler', () => {
    ...
    describe('with a valid session token', () => {
      ...
      it('only players in the same conversation area should receive join and update message', async () =>{
        ...
        mockReset(mockSocket1);
        const player1 = new Player(nanoid());
        const session1 = await testingTown.addPlayer(player1);
        const newLocation1:UserLocation = { moving: false, rotation: 'front', x: 25, y: 25, conversationLabel: newConversationArea.label };
        ...
        TestUtils.setSessionTokenAndTownID(testingTown.coveyTownID, session1.sessionToken, mockSocket1);
        townSubscriptionHandler(mockSocket1);
        TestUtils.setSessionTokenAndTownID(testingTown.coveyTownID, session2.sessionToken, mockSocket2);
        townSubscriptionHandler(mockSocket2);

        testingTown.onUpdate('hi', newConversationArea.label.concat(testingTown.coveyTownID));
        testingTown.onJoin(newConversationArea.label.concat(testingTown.coveyTownID),  newConversationArea.occupantsByID);
        
        // only player1 and player2 are able to recieve message because they are in the conversation area 
        // player should not recieve 
        expect(mockSocket1.emit).toBeCalledWith('canvas-data', 'hi');
        expect(mockSocket2.emit).toBeCalledWith('canvas-data', 'hi');
