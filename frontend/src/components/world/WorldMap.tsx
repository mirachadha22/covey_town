...
class CoveyGameScene extends Phaser.Scene {
  ...
  private setShowButton: (show: boolean) => void;
...
export default function WorldMap(): JSX.Element {
  ...
  const [currentConv, setCurrentConv] = useState<ConversationArea>();
  ...
  // pause the game when conversation modal displays  
  ...
  // get the conversation area where the current player is located in 
  useEffect(() => {
    let found = false;
    for (const conv of conversationAreas) {
      if (conv.occupants.filter(p => p === myPlayerID).length !== 0) {
        setCurrentConv(conv);
        found = true;
        setShowButton(true);
        break;
  ...
  }, [conversationAreas, gameScene, myPlayerID]); 
  ...
  // construct the whiteboard modal that pop out after click the button
  const whiteBoardModal = useMemo(() => {
    if (whiteBoardDisplay && currentConv) {
      video?.pauseGame();
      return (
        <WhiteBoardModal
          curConversationArea={currentConv}
          isOpen={whiteBoardDisplay !== undefined}
          curTownId={video?.coveyTownID}
          closeModal={() => {
            video?.unPauseGame();
    ...
    return <></>;
  }, [video, whiteBoardDisplay, setWhiteBoardDisplay, currentConv, gameScene]);
  ...
  return (
    <div id='app-container'>
      ...
      <div id='social-container'>
        {showButton ? (
          <Button onClick={() => {setWhiteBoardDisplay(true);
       ;}}> Open WhiteBoard </Button>
        ) : null}
        <SocialSidebar playerID={myPlayerID} setShowButton={setShowButton} setCurrentConv = {setCurrentConv} />
      </div>
    </div>
  );
}
